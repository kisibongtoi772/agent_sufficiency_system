...
- Graphify describes itself as a code knowledge graph that an assistant queries instead of grepping raw files; its documentation explicitly says it is **not an agent**. Its published token benchmark reports 71.5x fewer tokens/query on a 52-file mixed corpus versus naive full-context, while a 6-file corpus is about 1x. Source: https://graphify.com/what-is-graphify and https://github.com/Graphify-Labs/graphify/blob/v8/docs/how-it-works.md
- Graphify's July 2026 benchmark reports a code-agent test on ERPNext where adding one Graphify tool improved key-fact coverage from 70.8% to 82.0% on six graded questions, at roughly 140K tokens/query. This is vendor-run and n=6, so it is useful evidence but not definitive. Source: https://github.com/Graphify-Labs/graphify/blob/v8/BENCHMARKS.md
- LangGraph documentation positions LangGraph as graph/state orchestration: nodes read/write shared state, and the docs explicitly recommend keeping state raw and formatting prompts on demand. Therefore token efficiency depends on the context policy implemented inside nodes; LangGraph is not itself a retrieval compressor. Source: https://docs.langchain.com/oss/javascript/langgraph/thinking-in-langgraph
